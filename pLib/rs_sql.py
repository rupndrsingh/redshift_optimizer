# Custom module to connect to SQL DB and retrieve data
...
def load_config_file(cfgFile):
    """ Parsed content of Config file into Dictionary 
        ...
        port="port"
        ssl_key="path/to/.key" #required for remote connections 
        ssl_cert="path/to/.cer" #required for remote connections 
        ssl_ca="path/to/.pem" #required for remote connections 
    """
    with open(cfgFile) as f:
        content = f.readlines()
    cfg = {}
    primary_key = 0 # has primary key been discovered, which is the string enclosed in brackets in the config file)
    for line in content:
        if primary_key:
            if '=' in line:
                kv = line.split('=')
